...
class SmartMoneyConcepts:
            ...
            if closes[i] < opens[i]: # Down candle
                ...
                if subsequent_gain > (highs[i] - lows[i]) * 1.5:
                    obs.append({
                        ...
                        'tested': any(lows[k] <= highs[i] for k in range(i + 1, len(sub_df)))
                    })
            ...
            # Bearish Order Block: Bullish candle followed by strong bearish dump
                ...
                if subsequent_drop > (highs[i] - lows[i]) * 1.5:
                    obs.append({
                        ...
                        'timestamp': str(timestamps[i]),
                        'in_zone': (lows[i] <= current_price <= highs[i]),
                        'tested': any(highs[k] >= lows[i] for k in range(i + 1, len(sub_df)))
                    })
    ...
    def analyze_market_structure(df: pd.DataFrame, window: int = 5) -> Dict[str, Any]:
        ...
        return {
            ...
            'recent_swing_high': float(last_high),
            'recent_swing_low': float(last_low),
            'distance_to_high_pct': float(((last_high - current_close) / current_close) * 100.0),
            'distance_to_low_pct': float(((current_close - last_low) / current_close) * 100.0),
